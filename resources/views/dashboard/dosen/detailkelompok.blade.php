...
@section('content')
            ...
            <div class="row">
                <div class="col-12 d-flex justify-content-between">
                    ...
                    <button type="button" class="btn bg-danger my-2"
                        onclick="confirmKeluarKelompok('{{ route('dosen.keluar-kelompok', $informasiKelompok['id_kelompok']) }}')">
                        <i class="fa-solid fa-delete-left secondary-color me-2"></i><span class="secondary-color"> Keluar
                            Kelompok
                        </span>
                    </button>
                </div>
            ...
            @forelse ($judul as $item)
                <div class="card bg-primary-color h-100 my-3 rounded-4">
                    ...
                    <div class="collapse mx-3 mb-2" id="collapseExample{{ $item->id }}">
                                    ...
                                    <div class="commentar flex-inline gap-2 align-items-center">
                                        ...
                                        @if ($km->id_user == Auth::user()->id)
                                            ...
                                            <button class="btn btn-danger btn-sm"
                                                onclick="confirmDeleteKomentar('{{ route('dosen.delete-komentar', [$informasiKelompok['id_kelompok'], $km->id]) }}')">
                        ...
                        @endforelse
                        <form action="{{ route('dosen.post-komentar', [$item->id, $item->id_kelompok]) }}"
                            method="POST">
                            @csrf
    ...
    @include('dashboard.dosen.modal.editkomentar')
    <!-- Form Untuk Delete -->
    ...
    <script>
        function confirmKeluarKelompok(url) {
            Swal.fire({
                title: 'Anda Yakin ingin keluar dari kelompok?',
                text: "Anda tidak akan dapat mengembalikan ini!",
                icon: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#d33',
